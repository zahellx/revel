...
const { User, Token } = require('../../src/database');
const { roleRights } = require('../../src/constants/roles');
...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({
        name: newUser.name, email: newUser.email, role: 'user',
      });
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.get('jwt').refreshExpirationDays, 'days');
  ...
  describe('Auth middleware', () => {
    ...
    test('should call next with unauthorized error if the token is not an access token', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.get('jwt').accessExpirationMinutes, 'minutes');
      const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
